  ...
  (try
    (if-let [query-execution (:query-execution (ex-data e))]
      (merge (query-execution-info query-execution)
             (format-exception* (.getCause e) extra-info))
      (merge
...
(defn catch-exceptions
  ...
  (fn [query xformf {:keys [preprocessedf nativef raisef finishf], :as context}]
    {:pre [(fn? finishf)]}
    (let [extra-info (atom nil)]
      (letfn [(preprocessedf* [query]
              ...
              (raisef* [e]
                ...
                ;; NOCOMMIT - don't need both
                (finishf (format-exception* query e @extra-info))
                (format-exception* query e @extra-info)
                #_(throw (qp.reducible/quit (format-exception* query e @extra-info))))]
        (try
